...
// The object that we have to update:
...
const empId = route.params.paramId;
...
onMounted(async () => {
  try {
    const res = await axios.get(`http://localhost:3000/api/emp/${empId}`);
    updateObj.value = res.data;
    form.id = updateObj.value.id;
    form.name = updateObj.value.name;
    form.mobileNumber = updateObj.value.mobileNumber;
    form.designation = updateObj.value.designation;
    form.skills = updateObj.value.skills;
    form.gender = updateObj.value.gender;
    form.address = updateObj.value.address;
    form.state = updateObj.value.state;
    form.city = updateObj.value.city;
    form.talukas = updateObj.value.talukas;
    form.lastCompany = updateObj.value.lastCompany;
    form.salary = updateObj.value.salary;

    const selectedStateName = form.state;

    const AllStatesRes = await axios.get("http://localhost:3000/api/state");
    const selectedState = AllStatesRes.data.find(
      (item) => item.name === selectedStateName
    );
    // console.log(selectedState);
    stateId.value = selectedState._id;
    // console.log(stateId.value);

    const cityRes = await axios.get("http://localhost:3000/api/city", {
      params: { stateId: stateId.value },
    });
    ...
    const selectedCityName = form.city;
    const AllCityRes = cities.value.find(
      (item) => item.name === selectedCityName
    );
    cityId.value = AllCityRes._id;

    const AllTalukaRes = await axios.get("http://localhost:3000/api/taluka", {
      params: { cityId: cityId.value },
    });

    // console.log(AllTalukaRes.data);

    AllTalukas.value = AllTalukaRes.data;
  } catch (error) {
    console.log(error);
  }
});

// fetching city and taluka arrays
</script>

<template>
  <div class="updateEmployee">
    <div class="updateEmployeeWrapper">
